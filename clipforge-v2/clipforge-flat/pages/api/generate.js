...
export default async function handler(req, res) {
  ...
  if (!topic || topic.trim().length < 3) return res.status(400).json({ error: 'Please enter a video topic (at least 3 characters).' });
...
Respond with ALL these sections using the exact bold headers below:

**Video Title**: [compelling, click-worthy title]

**Hook**: [attention-grabbing opening line for first 3-5 seconds]

**Outline**: [structured breakdown with timestamps]

**Script**: [detailed sample script with natural dialogue]

**Thumbnail Concept**: [vivid thumbnail description - text, imagery, colors, composition]
...
**Tips**: [3 production tips for this specific video]
...
Be specific, creative, and platform-aware.`;
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return res.status(502).json({ error: err?.error?.message || 'AI service error. Please try again.' });
    }
