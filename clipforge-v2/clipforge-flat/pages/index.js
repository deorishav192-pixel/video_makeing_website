...
function parse(raw) {
  ...
  return {
    ...
    outline:   sec('Outline'),
    script:    sec('Script'),
    thumbnail: sec('Thumbnail Concept'),
    caption:   sec('Caption'),
    hashtags:  htags,
    music:     sec('Music'),
    tips:      sec('Tips'),
...
export default function Home() {
  ...
  const label = { display:'block', fontSize:'13px', fontWeight:'600', marginBottom:'7px' };
  const input = { width:'100%', background:'#111827', border:'1px solid #1e2a3a', borderRadius:'8px', color:'#f0f4ff', fontFamily:"'DM Sans',sans-serif", fontSize:'14px', padding:'10px 13px', outline:'none' };
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Generate complete video scripts, outlines, thumbnails & hashtags for YouTube and Instagram." />
      </Head>
      ...
      <div style={{position:'relative',zIndex:1,minHeight:'100vh',display:'flex',flexDirection:'column'}}>
        ...
        <section style={{padding:'56px 40px 32px',textAlign:'center',maxWidth:'860px',margin:'0 auto'}}>
          ...
          <p style={{color:'#6b7a99',fontSize:'16px',fontWeight:'300',lineHeight:'1.65',maxWidth:'460px',margin:'0 auto'}}>
            Complete scripts, outlines, thumbnails &amp; hashtag strategies for YouTube &amp; Instagram.
          </p>
        </section>
        ...
        <div style={{display:'grid',gridTemplateColumns:'360px 1fr',flex:1,borderTop:'1px solid #1e2a3a'}}>
          ...
          <aside style={{borderRight:'1px solid #1e2a3a',padding:'28px',background:'#0e1320',overflowY:'auto'}}>
            ...
            <div style={{marginBottom:'24px'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                {['YouTube','Instagram','Both'].map(p => (
                  <button key={p} onClick={()=>{setPlatform(p);setDuration('');}}
                    style={{background: platform===p ? 'rgba(232,255,60,0.08)':'#111827', border:`2px solid ${platform===p?'#e8ff3c':'#1e2a3a'}`,borderRadius:'10px',padding:'13px 10px',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:'6px',color:platform===p?'#e8ff3c':'#6b7a99',fontFamily:"'DM Sans',sans-serif",fontSize:'12px',fontWeight:'600',transition:'all 0.2s',gridColumn:p==='Both'?'1/-1':'auto'}}>
            ...
            <div style={{marginBottom:'24px'}}>
              ...
              <div style={field}>
                <label style={label}>Topic <span style={{color:'#e8ff3c'}}>*</span></label>
                <input type="text" placeholder="e.g. Morning routine for productivity" value={topic}
                  ...
                  maxLength={300} style={input} />
              </div>
              <div style={field}>
                <label style={label}>Genre</label>
                <select value={genre} onChange={e=>setGenre(e.target.value)} style={{...input,cursor:'pointer'}}>
                  {GENRES.map(g=><option key={g}>{g}</option>)}
                </select>
              </div>
              <div style={field}>
                <label style={label}>Target Audience <span style={{color:'#6b7a99',fontWeight:'400',fontSize:'12px'}}>(optional)</span></label>
                <input type="text" placeholder="e.g. Busy professionals 25–40" value={audience}
                  onChange={e=>setAudience(e.target.value)} maxLength={200} style={input} />
              </div>
            </div>
            ...
            <div style={{marginBottom:'24px'}}>
                ...
                <div style={{display:'flex',flexWrap:'wrap',gap:'6px'}}>
                  {TONES.map(t=>(
                    <button key={t} onClick={()=>toggleTone(t)} type="button"
                      style={{background:tones.includes(t)?'rgba(60,255,224,0.1)':'#111827',border:`1px solid ${tones.includes(t)?'#3cffe0':'#1e2a3a'}`,borderRadius:'20px',padding:'5px 13px',cursor:'pointer',fontSize:'12px',fontWeight:'600',color:tones.includes(t)?'#3cffe0':'#6b7a99',fontFamily:"'DM Sans',sans-serif",transition:'all 0.18s'}}>
                      {t}
                    </button>
                  ))}
                </div>
                ...
                <div style={{display:'flex',gap:'6px'}}>
                  {durs.map(d=>(
                    <button key={d} onClick={()=>setDuration(d)} type="button"
                      style={{flex:1,background:duration===d?'rgba(232,255,60,0.08)':'#111827',border:`1px solid ${duration===d?'#e8ff3c':'#1e2a3a'}`,borderRadius:'8px',padding:'8px 4px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:'11px',fontWeight:'600',color:duration===d?'#e8ff3c':'#6b7a99',transition:'all 0.18s'}}>
                      {d}
                    </button>
                  ))}
                </div>
              ...
              <div style={field}>
                <label style={label}>Special Notes <span style={{color:'#6b7a99',fontWeight:'400',fontSize:'12px'}}>(optional)</span></label>
            ...
            <button onClick={generate} disabled={loading} type="button"
              style={{width:'100%',padding:'15px',borderRadius:'12px',border:'none',cursor:loading?'not-allowed':'pointer',background:'#e8ff3c',color:'#080b12',fontFamily:"'Bebas Neue',sans-serif",fontSize:'20px',letterSpacing:'3px',position:'relative',overflow:'hidden',opacity:loading?0.6:1,transition:'transform 0.2s, box-shadow 0.2s'}}>
          ...
          <main style={{padding:'36px',overflowY:'auto',background:'#080b12'}}>
            ...
            {loading && (
              <div style={{minHeight:'400px',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'14px',textAlign:'center'}}>
                ...
                <p style={{fontSize:'13px',color:'#6b7a99',fontWeight:'300'}}>Building script, outline, thumbnail &amp; more…</p>
            ...
            {result && !loading && (
              <div>
                <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'18px'}}>
                  <button onClick={generate} type="button" style={{background:'transparent',border:'1px solid #1e2a3a',color:'#6b7a99',borderRadius:'8px',padding:'7px 16px',cursor:'pointer',fontSize:'12px',fontWeight:'600',fontFamily:"'DM Sans',sans-serif"}}>↺ Regenerate</button>
                  <span style={{fontSize:'12px',color:'#6b7a99'}}>AI-generated — refine as needed</span>
                </div>

                <div style={{background:'#111827',border:'1px solid #1e2a3a',borderRadius:'16px',overflow:'hidden',animation:'fadeUp 0.45s ease'}}>
                  ...
                  <div style={{padding:'22px 26px',borderBottom:'1px solid #1e2a3a',background:'linear-gradient(135deg,rgba(232,255,60,0.04),transparent)',display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'16px'}}>
                    ...
                    <button onClick={copyAll} type="button" style={{background:'#0e1320',border:'1px solid #1e2a3a',color:'#6b7a99',borderRadius:'8px',padding:'7px 14px',cursor:'pointer',fontSize:'12px',fontWeight:'600',whiteSpace:'nowrap',fontFamily:"'DM Sans',sans-serif",flexShrink:0}}>
                  ...
                  <div style={{padding:'26px'}}>
                    ...
                    <div style={{display:'flex',gap:'10px',marginTop:'24px',paddingTop:'24px',borderTop:'1px solid #1e2a3a',flexWrap:'wrap'}}>
                      <button onClick={copyAll} type="button" style={{flex:1,minWidth:'140px',padding:'12px',borderRadius:'8px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:'13px',fontWeight:'600',background:'#e8ff3c',color:'#080b12',border:'1px solid #e8ff3c',transition:'all 0.2s'}}>
                        {copied?'✓ Copied!':'📋 Copy Full Concept'}
                      </button>
                      <button onClick={generate} type="button" style={{flex:1,minWidth:'140px',padding:'12px',borderRadius:'8px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:'13px',fontWeight:'600',background:'transparent',color:'#6b7a99',border:'1px solid #1e2a3a',transition:'all 0.2s'}}>
                        ↺ Regenerate
                      </button>
                    </div>
                  </div>
        ...
        <footer style={{textAlign:'center',padding:'18px',borderTop:'1px solid #1e2a3a',fontSize:'12px',color:'#6b7a99',letterSpacing:'0.5px',background:'#0e1320'}}>
